...
Class Users extends DBConnection {
	...
	public function save_users(){
		extract($_POST);
		$data = '';
		if (!empty($current_password) && !empty($id)) {
			$current_password = md5($current_password);
			$check_password = $this->conn->query("SELECT * FROM `users` WHERE id = $id AND password = '$current_password'")->num_rows;
		...
		foreach($_POST as $k => $v){
			if(!in_array($k, array('id','password', 'current_password'))){
				if(!empty($data)) $data .=" , ";
				$data .= " {$k} = '{$v}' ";
			}
		}
